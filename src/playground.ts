...
// let cats: string[] = []; //los arrays tienen que ser inicializados
...
// let arrPoli: any[] = []; //Any me permite meter cualquier chota dentro de un array
...
// sayHello(); //esto me deja enviarle un parametro aunque la funcion no lo pida... entonces
//la forma correcta de trabajar es haciendo que la funcion sea explicita y pida parametros (si los necesita)
...
//c? quiere decir que puede o no recibir un parametro
...
//INTERFACES
//EN TS una interfaz es un TIPO abstracto que indica al compilador qué props puede tener un objeto determinado. La interfaz no crea un objeto a su imagen y semejanza... apenas describe la forma que debe tener el objeto.
//En el caso de una interfaz como se ve aca abajo, el metodo no tiene una accion en particular, solo se que es del tipo void(no devuelve
//nada en particular), ahora... si yo creo un objeto de esta interfaz puedo crearle yo mismo una funcionalidad a ese metodo, una clase
//no me permitiria este comportamiento
interface isHero {
  name: string;
  age: number;
  isActive: boolean;
  powers: string[];
  catchPhrase(text: string): void;
}
...
const batman: isHero = {
  ...
  catchPhrase(phrase: string): void {
...
robin = {
  ...
  catchPhrase(message: string): void {
    console.log(`${this.name} says: "${message}"`);
  },
};
robin.name = "Piñon Fijo"; //esto no cambia la interfaz
robin.catchPhrase("Si la bici me responde, santos payasos Batman");

//CLASES QUE IMPLEMENTAN UNA INTERFAZ
//Hasta ahora hicimos una interfaz y creamos objetos literales que la implementaban, es decir, qu estaban obligados a una forma determinada. Ahora lo haremos a través de una clase que implemente la interfaz.
interface TieneFormato {
  format(): string;
}

class Factura implements TieneFormato {
  constructor(
...
//esto captura lo que entra a la funcion, sin importar lo que manden. Ademas de capturar el objeto captura el TIPO de dato entrante
const addUIDPlus = <T>(obj: T) => { 
  const uid: number = Math.round(Math.random() * 100000);
  return { ...obj, uid };
};
...
console.log(addUIDPlus("Hola Tarolas")); //el objeto lo que esta haciendo es desestructurar lo que mando... al desestructurar un string
//termino teniendo un array

//Podemos indicarle al tipo genérico que solamente reciba un objeto.
const addUIDPlusPlus = <T extends object>(obj: T) => {
  const uid: number = Math.round(Math.random() * 100000);
  return { ...obj, uid };
};
...
const addUIDPlusPlusBis = <T extends { name: string }>(obj: T) => {
  const uid: number = Math.round(Math.random() * 100000);
  return { ...obj, uid };
};
addUIDPlusPlusBis({ name: "Margaret", email: "marga123@correo.ar" });

//GENERCIS WITH INTERFACE
interface Resource {
  uid: number;
  resourceName: string;
  data: object;
...
const rsc01: Resource = {
  uid: 1,
  resourceName: "posts",
  data: { title: "El general tiene quien le escriba" },
};

//Si necesitamos cierta flexibilidad en el objeto (que data no sea obligatoriamente un objeto, por ej) podemos recurrir a tipos genéricos.
...
enum ResourceType {
  ...
  AUTHORS, //1
  FILMS, //2
} //CADA KEYWORD ESTÁ ASOCIADA AL ÍNDEX

interface ResourceBis<T> {
  uid: number;
  resourceType: ResourceType;
  data: T;
}

const rsc04: ResourceBis<string[]> = {
  uid: 67,
  resourceType: ResourceType.BOOKS, //mirar lo que hay dentro de books => 0, el indice
...
//al laburar con un array no tengo un orden definido, solo el tipo de dato que puede entrar
const arreglo: (number | boolean | string)[] = [15, false, "una palabra"];
...
//si quiero agregar mas cosas dentro de la tupla puedo, pero tiene Q respetar alguno de los TIPOS de datos definidos
const miTupla: [number, string, boolean] = [90, "un texto", true]; 
console.log(miTupla);

miTupla.pop();
// miTupla.push(56); //Si yo agregara un number despues de haber borrado el ultimo dato (boolean) lo hagarra igual, no causa error
console.log(miTupla);
